...
object Essentials {
  ...
  // inheritance mode: extend <= 1 class, but inherit from >= 0 traits
  ...
  // pattern matching
  val anUnkonwn: Any = 45
  val ordinal = anUnkonwn match {
    case 1 => "first"
    case 2 => "second"
    case _ => "unknown"
  }
  ...
  // Future
  // import scala.concurrent.ExecutionContext.Implicits.global // before 2.13
  implicit val ec: ExecutionContext = ExecutionContext.fromExecutorService(Executors.newFixedThreadPool(8))
  val aFuture = Future{
    42
  }

  // wait for completion (async) - takes partial function
  aFuture.onComplete {
    case Success(value) => println(s"The async meaning od life is $value")
  ...
  // partial functions - accepts values only that pattern match
  val abstractPartialFunction: PartialFunction[Int, Int] = {
    case 1 => 43
    case 2 => 56
    case 100 => 999
  }
