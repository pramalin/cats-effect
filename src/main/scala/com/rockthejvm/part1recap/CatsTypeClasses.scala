...
object CatsTypeClasses {
  ...
  // here Functor is context constraint
  ...
  val aSimoleList_v2: List[Int] = 43.pure[List]
  ...
  def crossProduct[F[_], A, B](fa: F[A], fb: F[B])(using flatMap: FlatMap[F]): F[(A, B)] =
    fa.flatMap(a => fb.map(b => (a, b))) // map is also available because FlatMap extends Functor

  // Monad - applicative + flatMap  a structure that cobines functional programming and imperative programming
  ...
  import cats.syntax.applicativeError.* // raiseError extension methods
  ...
  // turn nested wrapper inside out
