...
object CountdownLatches extends IOApp.Simple {
  ...
  /*
    CDLatches are coordination primitive initialized with a count.
    All fibers calling await() on the CDLatch are (semantically) blocked.
    When the internal count of the latch reaches 0 (via release() calls from other fibers), all the awaiting fibers are unblocked.
   */
...
/* my attempt
  def downloadFile(filename: String, destFolder: String): IO[Unit] = for {
    ...
    contents <- (1 to numChunks).toList.parTraverse{n =>
      for {
    ...
    _ <- latch.await
    tmpFile <- appendFileContents("tmp-file", contents) ???
