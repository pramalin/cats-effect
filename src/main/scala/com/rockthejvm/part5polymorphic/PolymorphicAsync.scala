...
object PolymorphicAsync extends IOApp.Simple {
  ...
  trait MyAsync[F[_]] extends Sync[F] with Temporal[F] {
    ...
    def never[A]: F[A]  = async_(cb => ())// never-ending effect
  }

  val asyncIO = Async[IO] // given AsyncIO

  // pure, map/flatMap, raiseError, uncancellable, start, ref/deferred, sleep, delay/defer/blocking
  // +
  ...
  val asyncMeaningOfLifeComplex: IO[Int] = IO.async { (cb: Callback[Int]) =>
    ...
    }.as(Some(IO("Cancelled").debug.void)) // finalizer in case the computation gets canceled
  }
  ...
  val asyncMeaingOfLife_v3 = asyncIO.evalOn(IO(42).debug, myExecutionCOntext).guarantee(IO(threadPool.shutdown()))
